...
BOOST_AUTO_TEST_CASE(test_tree) {
    ...
    SymbolCodeMap code_map = pair.first;
    ...
    pair = generateHuffmanCode({ 123 });
    SymbolCodeMap code_map2 = pair.first;
    BOOST_CHECK(equals(code_map2[123], Bitstream({ 0 })));
    BOOST_CHECK_EQUAL(code_map2.size(), 1);

    SymbolsPerLength symbols = pair.second;
    BOOST_CHECK_EQUAL(symbols.size(), 17);
    BOOST_CHECK(symbols[1] == vector<int>({ 123 }));
}
